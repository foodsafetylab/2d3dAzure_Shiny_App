    ...
    sidebarMenu(
        ...
        menuItem(text = "Inputs", 
                 ...
                 menuItem(text = "Manual Version", 
                          tabName = "v_manual", 
                          menuSubItem(text = "1D (Powder Production)", tabName = "1D"),
                          menuSubItem(text = "2D (Produce Field)", tabName = "2D"),
                          menuSubItem(text = "3D (Corn Bin)", tabName = "3D")),
                 tabName = "inputs"),
...
v_manual_1D = fluidRow(
    ...
    box(title = "Powder Sampling Input Parameters", solidHeader = TRUE, status = "primary",
        fluidRow(
            column(
                width = 10, 
                actionButton("OneD_Scenario_Recalled", "Recalled Batch Scenario"),
                actionButton("OneD_Scenario_Reference", "Reference Batch Scenario"),
                style = "padding-right: 5px;"
            ),
            column(width = 2, 
                   actionButton("Doc_Scenario_1D", "",icon = icon("question-circle"), style = "padding: 5px; margin: 0px 0px 0px 0px;" )
            )
        ),
        p(),
        ...
        fluidRow(
            column(width = 10, 
                   splitLayout(
                       numericInput(inputId = "x_lim_1d", label = "Production Time (h)", value = 20, min = 1),
                       numericInput(inputId = "y_lim_1d", label = "Production Rate (tonne/h)", value = 1.1, min = 1)
                   ), style = "padding-right: 0px;")),
        fluidRow(
            column(width = 10, 
                   splitLayout(
                       numericInput(inputId = "spread_radius_1d", label = "Contamination proportion, prevalence (%)", value = 42, min = 0)
                   ),style = "padding-right: 0px;"),
            column(width = 2, 
                   actionButton("Prevalence_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;",icon = icon("question-circle") )
            )
        ), 
        fluidRow(
            column(width = 10, 
                   splitLayout(
                       numericInput(inputId = "cont_level_mu_1d", label = "Mean contamination level (log CFU/g)", value = -1.779, step = 0.001),
                       numericInput(inputId = "cont_level_sd_1d", label = "Standard deviation of contamination level (log CFU/g)", value = 0.675, step = 0.001)
                   ),style = "padding-right: 0px;"),
            column(width = 2, 
                   actionButton("ContLevels_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;", icon = icon("question-circle") )
            )
        ), 
        fluidRow(
            column(width = 10, 
                   selectInput(inputId = "method_sp_1d", label = "Sampling strategy", choices = list("Simple Random" = "srs", "Stratified Random" = "strs", "Systematic" = "ss")), value = "ss", style = "padding-right: 0px;"),
            column(width = 2, 
                   actionButton("Sampling_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;",icon = icon("question-circle") )
            )
        ),
        conditionalPanel(
            ...
            splitLayout(
                fluidRow(
                    column(width = 10, 
                           numericInput(inputId = "n_sp_1d", label = "Number of sample points", value = 30, min = 1, step = 1),style = "padding-right: 0px;"),
                    column(width = 2, 
                           actionButton("SamplePoints_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;",icon = icon("question-circle") )
                    )
                ),
            )
        ),
        conditionalPanel(
            ...
            splitLayout(
                fluidRow(
                    column(width = 10, 
                           numericInput(inputId = "n_strata_1d", label = "Number of strata", value = 30, min = 1, step = 1),style = "padding-right: 0px;"),
                    column(width = 2, 
                           actionButton("NStrata_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;",icon = icon("question-circle") )
                    )
                ),
                fluidRow(
                    column(width = 10, 
                           numericInput(inputId = "n_sp_ps_1d", label = "Number of sample points per strata", value = 1, min = 1, step = 1),style = "padding-right: 0px;"),
                    column(width = 2, 
                           actionButton("SamplesNStrata_1D", "",style = "padding: 5px; margin: 26px 0px 0px 0px;",icon = icon("question-circle") )
                    )
                ),
            )
        ),
        numericInput(inputId = "m_sp_1d", label = "Individual grab mass (g)", value = 10, min = 0),
        h3(),
        h2("Iteration section"),
        splitLayout(
            numericInput(inputId = "n_seed_1d", label = "Number of contamination patterns", value = 10, min = 1, step = 1),
            numericInput(inputId = "n_iter_1d", label = "Number of sampling patterns per contamination pattern", value = 100, min = 1, step = 1)
        ),
...
page_intro = fluidRow(box(width = 12,
                          ...
                          p(),
                          h2("Goal"),
                          p("We aim to provide a tool to simulate bulk sampling in 1D (e.g. Dairy Powders), 2D (e.g. a produce field) or 3D (e.g. a grain container) scenarios and evaluate the performance of any specific sampling plan."),
                          p(),
                          h2("Funding Sources"),
                          tags$div("This web app is the product of three research projects funded by ", 
                                   tags$a(href = "https://ilsi.org/", "International Life Sciences Institute"),
                                   " (ILSI),",
                                   tags$a(href = "https://iafns.org/", "The Institute for the Advancement of Food and Nutrition Sciences"),
                                   " (IAFNS), and",
                          ...
                          tags$ul(
                              tags$li(tags$div("Cheng, X., Stasiewicz, M. J. (2021). Evaluation of the impact of skewness, clustering, and probe sampling plan on aflatoxin detection in corn. Risk Analysis. ",
                                               tags$a(href = "https://doi.org/10.1111/risa.13721", "Link"))),
                              ...
                              tags$li(tags$div("Quintanilla Portillo J., Cheng X., Belias A.M., Weller D.L., Wiedmann M., Stasiewicz, M. J. (2023). A Validated Preharvest Sampling Simulation Shows that Sampling Plans with a Larger Number of Randomly Located Samples Perform Better than Typical Sampling Plans in Detecting Representative Point-Source and Widespread Hazards in Leafy Green Fields",
                                               tags$a(href = "https://doi.org/10.1128/aem.01015-22", "Link"))),
                              tags$li(tags$div("Kim M.,Reyes G.A., Cheng X., Stasiewicz, M. J. (2023). Simulation Evaluation of Power of Sampling Plans to Detect Cronobacter in Powdered Infant Formula Production",
                                               tags$a(href = "https://doi.org/10.1016/j.jfp.2023.100115", "Link")))
                              
                          ),
                          p(),
                          h2("Version"),
                          tags$div("The current version is 4.0.2 and was updated on: 06/16/2023. Previous versions and source code can be found on GitHub (",
                                   tags$a(href = "https://github.com/foodsafetylab", "link"),
                                   ")."),
                          p(),
                          h2("Contacts"),
                          ...
                          tags$div(tags$b("Xianbin (Eric) Cheng"), "Corn Sampling Lead, Email: ericxbcheng@gmail.com", tags$a(href = "https://www.linkedin.com/in/xianbincheng/", " LinkedIn")),
                          tags$div(tags$b("Jorge Quintanilla"), ": Produce Sampling Lead. Email: jfq@illinois.edu"),
                          tags$div(tags$b("Minho Kim"), ": Powder Sampling Lead. Email: minho3@illinois.edu")
))
